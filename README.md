...
```bash
sudo bash setup_mtproto.sh example.com          # только домен
sudo bash setup_mtproto.sh example.com 443      # свой порт
sudo bash setup_mtproto.sh example.com 2083 3129  # все параметры
sudo bash setup_mtproto.sh 1.2.3.4              # без домена, по IP
```
...
Скрипт автоматически настраивает UFW если он активен. Если UFW выключен — в конце установки будет выведен **готовый блок команд** для ручного включения.

> ⚠️ **Важно:** многие VDS-провайдеры имеют **внешний firewall в панели управления** — он не зависит от UFW на сервере. Если после установки коннекта нет, открой порт `2083/tcp` в панели управления своим сервером у провайдера.

Проверить доступность порта снаружи:
```bash
# С другой машины или Mac:
nc -zv YOUR_SERVER_IP 2083
```
...
```bash
docker ps | grep mtg          # статус контейнера
docker logs -f mtg            # логи
docker restart mtg            # перезапуск
docker stop mtg               # остановить
cat /opt/mtg/secret.txt       # сохранённый секрет
```
...
- Root-доступ (`sudo`)
- Открытый порт `2083/tcp` (или свой)
...
```bash
docker logs mtg               # логи контейнера
ss -tlnp | grep 2083          # слушается ли порт
ufw status                    # статус firewall
sudo lsof -i :2083            # что занимает порт
```
